...
// MongoDB Connection
mongoose
  .connect(process.env.MONGO_URI || "mongodb+srv://jobaerafroz4:qwerty123456@cluster0.vp15qty.mongodb.net/")
...
app.post("/register", async (req, res) => {
  try {
    ...
    if (!varsityId || !fullName || !email || !password) {
      return res
        .status(400)
        .json({ message: "All fields (varsityId, fullName, email, password) are required" });
    }
    ...
    const emailSent = await sendVerificationEmail(newUser.email, newUser.verificationToken);
    if (!emailSent) {
